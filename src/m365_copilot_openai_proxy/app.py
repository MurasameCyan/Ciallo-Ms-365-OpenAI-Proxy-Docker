...
function donut(parts,centerLabel,centerVal){
  ...
  if(total>0){
    ...
    visibleParts.forEach((p,i)=>{
      ...
      const startA=(off/C)*Math.PI*2-Math.PI/2,endA=((off+len)/C)*Math.PI*2-Math.PI/2,large=ratio>.5?1:0;
      const sx=60+Math.cos(startA)*R,sy=60+Math.sin(startA)*R,ex=60+Math.cos(endA)*R,ey=60+Math.sin(endA)*R;
      const d='M '+sx.toFixed(2)+' '+sy.toFixed(2)+' A '+R+' '+R+' 0 '+large+' 1 '+ex.toFixed(2)+' '+ey.toFixed(2);
      ring+='<path d="'+d+'" fill="none" stroke="'+p.color+'" stroke-width="15" stroke-linecap="butt" filter="url(#'+uid+'sh)" opacity="0.96"><animate attributeName="opacity" values="0.84;1;0.84" dur="5.5s" repeatCount="indefinite"/><animate attributeName="stroke-width" values="14;16.5;14" dur="5.5s" repeatCount="indefinite"/></path><circle cx="'+sx.toFixed(2)+'" cy="'+sy.toFixed(2)+'" r="7.5" fill="'+p.color+'" filter="url(#'+uid+'sh)" opacity="0.96"><animate attributeName="opacity" values="0.84;1;0.84" dur="5.5s" repeatCount="indefinite"/></circle><circle cx="'+ex.toFixed(2)+'" cy="'+ey.toFixed(2)+'" r="7.5" fill="'+p.color+'" filter="url(#'+uid+'sh)" opacity="0.96"><animate attributeName="opacity" values="0.84;1;0.84" dur="5.5s" repeatCount="indefinite"/></circle>';
