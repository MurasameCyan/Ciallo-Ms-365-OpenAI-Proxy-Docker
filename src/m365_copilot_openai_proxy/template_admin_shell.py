...
<label class="runtime-field-label"><span data-i18n="cloud_cleanup_idle_hours_label">云端对话闲置回收（小时）</span><input id="runtime-cloud-cleanup-idle-hours" type="number" min="0" style="margin-top:.4rem;width:100%;box-sizing:border-box;padding:8px 10px;background:var(--inner);border:1px solid var(--inner-border);border-radius:8px;color:var(--strong)"><span style="display:block;font-size:.75rem;color:var(--faint);margin-top:.3rem" data-i18n="auto_cleanup_hint">0 表示关闭该项。本地会话被回收后，同一对话的下一轮会开一条没有历史的新上游会话；云端回收会删除该账户下所有没有本地会话引用的旧对话，包括账户主人自己在 Copilot 网页里聊的，请谨慎开启。</span></label>
</div>
<div style="display:grid;gap:.7rem">
...
<label class="runtime-field-label"><span data-i18n="proxy_url_label">出站代理</span><input id="runtime-proxy-url" placeholder="socks5h://127.0.0.1:1080" style="margin-top:.4rem;width:100%;box-sizing:border-box;padding:8px 10px;background:var(--inner);border:1px solid var(--inner-border);border-radius:8px;color:var(--strong)"><span style="display:block;font-size:.75rem;color:var(--faint);margin-top:.3rem" data-i18n="proxy_url_hint">留空为直连。用于服务器无法直接访问 M365 的部署（如中国大陆）。本地 CDP 始终直连，不走代理。</span></label>
</div>
</div>
