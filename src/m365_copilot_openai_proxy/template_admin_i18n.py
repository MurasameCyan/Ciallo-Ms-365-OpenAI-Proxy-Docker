...
_ADMIN_I18N_JS = """const i18n={
  zh:{
    ...
    pkce_start:'浏览器登录',pkce_finish:'完成登录',pkce_paste_ph:'把浏览器登录后地址栏里的 URL 粘到这里（含 ?code=...）',
    pkce_mint_media:'取附件 key',pkce_mint_designer:'取图片 key',
    pkce_hint:'登录一次即可：拿到的是原生客户端的滑动 refresh_token，之后 token / 附件 key / 图片 key 全走 HTTP 续期，不再依赖 cookie 和无头浏览器。',
    ...
    pkce_keys_ok:'附件 key 与图片 key 已一并铸造。',pkce_keys_failed:'但这些 key 没取到：{kinds}，可稍后单独重试。',
    pkce_minting:'正在用 refresh_token 换 key...',pkce_minted:'{kind} key 已更新（{shape}）',pkce_opaque:'加密令牌',
  ...
  en:{
    ...
    pkce_start:'Sign in',pkce_finish:'Finish',pkce_paste_ph:'Paste the URL from the address bar after signing in (the one with ?code=...)',
    pkce_mint_media:'File key',pkce_mint_designer:'Image key',
    pkce_hint:'One sign-in is enough: it returns the native client\\u0027s sliding refresh token, after which the token, file key and image key are all renewed over HTTP -- no cookies, no headless browser.',
    ...
    pkce_keys_ok:'File key and image key were minted too.',pkce_keys_failed:'but these keys could not be minted: {kinds} -- retry them separately.',
    pkce_minting:'Exchanging the refresh token for a key...',pkce_minted:'{kind} key updated ({shape})',pkce_opaque:'encrypted token',
