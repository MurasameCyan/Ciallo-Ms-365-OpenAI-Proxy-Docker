...
_ADMIN_I18N_JS = """const i18n={
  zh:{
    ...
    runtime_title:'运行设置（全局模板）',time_zone_label:'时区',auto_refresh_label:'自动刷新',run_permission_label:'运行权限',run_permission_inherit:'继承全局',run_permission_read_only:'只读',run_permission_full:'完全',tool_planning_label:'工具调用规划',tool_planning_auto:'自动',tool_planning_native:'内联契约',tool_planning_router:'路由模式',tool_planning_hint_auto:'只对实测不遵守契约的模式加一轮路由判定，其余模式不额外花轮数（默认）。',tool_planning_hint_native:'契约写进提示词，永不多花轮数；模式不遵守时这一轮就没有工具调用。',tool_planning_hint_router:'每轮先判定要不要调工具，判「不需要」也要多花一次上游往返。',refresh_before_label:'提前刷新秒数',idle_timeout_label:'空闲超时分钟',ws_idle_timeout_label:'对话响应超时分钟',keepalive_check_label:'保活检查间隔（分钟）',cookie_keepalive_before_label:'Cookie 提前保活（小时）',auto_cleanup_minutes_label:'自动回收间隔（分钟）',session_idle_hours_label:'本地会话闲置回收（小时）',cloud_cleanup_idle_hours_label:'云端对话闲置回收（小时）',auto_cleanup_hint:'0 表示关闭该项。本地会话被回收后，同一对话的下一轮会开一条没有历史的新上游会话；云端回收会删除该账户下所有没有本地会话引用的旧对话，包括账户主人自己在 Copilot 网页里聊的，请谨慎开启。',ports_logs_title:'端口与日志',cdp_port_label:'CDP 主端口',account_cdp_port_base_label:'CDP 用户端口',log_level_label:'日志等级',call_log_limit_label:'调用记录上限',user_log_verbose_label:'用户运行日志',user_log_errors_label:'用户错误日志',suppress_access_log_label:'访问日志屏蔽',rate_limit_rpm_label:'速率限制（次/分）',rate_limit_burst_label:'突发容量',account_concurrency_label:'账户并发上限',proxy_url_label:'出站代理',proxy_url_hint:'留空为直连。用于服务器无法直接访问 M365 的部署（如中国大陆）。本地 CDP 始终直连，不走代理。',
    tone_saved:'已保存',
